...
local gameFolder = "Net X/" .. gameName
...
function SetAutoload(configName)
    if writefile then
        ...
        print("[NTX] Autoload config set to: " .. configName)
    end
end
...
function ClearAutoload()
    if delfile and isfile(AutoloadFile) then
        ...
        print("[NTX] Autoload config cleared")
    end
end
...
function SaveConfigAs(configName)
    if not configName or configName == "" then
        warn("[NTX] Config name cannot be empty")
        return false
    end
    ...
    if writefile then
        ...
        print("[NTX] Config saved as: " .. configName)
        return true
    end
    return false
end
    ...
    if isfile and isfile(filepath) then
        ...
        if success and type(result) == "table" then
            if result._version == CURRENT_VERSION then
                ...
                CurrentConfigName = configName
                LoadConfigElements()
                print("[NTX] Config loaded: " .. configName)
                return true
            else
                warn("[NTX] Config version mismatch")
                return false
            end
        else
            warn("[NTX] Failed to decode config")
            return false
        end
    else
        warn("[NTX] Config file not found: " .. configName)
        return false
    end
end
...
function DeleteConfig(configName)
    ...
    if delfile and isfile(filepath) then
        delfile(filepath)
        print("[NTX] Config deleted: " .. configName)
...
function LoadExternalConfigFromJSON(jsonString)
    ...
    if not ok or type(data) ~= "table" then
        warn("[NTX] External config JSON invalid")
        return false
    end

    if data._version and CURRENT_VERSION and data._version ~= CURRENT_VERSION then
        warn("[NTX] External config version mismatch")
        return false
    end
    ...
    print("[NTX] External config loaded from JSON input")
    return true
end
...
function Netx:MakeNotify(NotifyConfig)
    NotifyConfig = NotifyConfig or {}
    NotifyConfig.Title = NotifyConfig.Title or "Netx"
...
function netx(msg, delay, color, title, desc)
    return Netx:MakeNotify({
        Title = title or "Net X",
...
local function ShowLoading()
    ...
    icon.Image = "rbxassetid://94533770468482"
    icon.Parent = bg
    ...
    msg.Text = "Net X Script are free script!"
...
function Netx:Window(GuiConfig)
    if not _G.__NTX_LOADED then
        _G.__NTX_LOADED = true
        ShowLoading()
        task.wait()
    end

    GuiConfig              = GuiConfig or {}
    GuiConfig.Title        = GuiConfig.Title or "My Script"
    GuiConfig.Footer       = GuiConfig.Footer or "Description"
            ...
            function Items:AddConfigPanel()
                    ...
                    Callback = function()
                        if selectedConfigName ~= "" then
                            if SaveConfigAs(selectedConfigName) then
                                netx("Config saved: " .. selectedConfigName, 3, Color3.fromRGB(100, 255, 100))
                            ...
                            netx("Please enter a config name", 3, Color3.fromRGB(255, 100, 100))
                        end
                    end,
                    SubCallback = function()
                        if selectedConfigName ~= "" then
                            if LoadConfig(selectedConfigName) then
                                netx("Config loaded: " .. selectedConfigName, 3, Color3.fromRGB(100, 255, 100))
                                StatusParagraph:SetContent("Current: " .. selectedConfigName .. " | Autoload: " .. (GetAutoload() or "None"))
                            else
                                netx("Failed to load: " .. selectedConfigName, 3, Color3.fromRGB(255, 100, 100))
                            end
                        else
                            netx("Please select a config", 3, Color3.fromRGB(255, 100, 100))
                        end
                    end
                })
                    ...
                    Callback = function()
                        if selectedConfigName ~= "" then
                            ...
                            if DeleteConfig(selectedConfigName) then
                                netx("Config deleted: " .. selectedConfigName, 3, Color3.fromRGB(255, 100, 100))
                                selectedConfigName = ""
                                ConfigNameInput:Set("")
                                local newConfigs = GetConfigList()
                                ConfigDropdown:Set(newConfigs)
                            ...
                            netx("Please select a config to delete", 3, Color3.fromRGB(255, 100, 100))
                        end
                    end,
                    SubCallback = function()
                        if selectedConfigName ~= "" then
                            SetAutoload(selectedConfigName)
                            netx("Autoload set to: " .. selectedConfigName, 3, Color3.fromRGB(100, 200, 255))
                ...
                Items:AddButton({
                    ...
                    Callback = function()
                        local newConfigs = GetConfigList()
                        ConfigDropdown:Set(newConfigs)
                        netx("Config list refreshed (" .. #newConfigs .. " configs)", 2, Color3.fromRGB(150, 150, 255))
                    end,
                    SubCallback = function()
                        ClearAutoload()
                        netx("Autoload cleared", 3, Color3.fromRGB(200, 200, 100))
                ...
                Items:AddButton({
                    ...
                    Callback = function()
                        ResetElements()
                        netx("All elements reset to default values", 3, Color3.fromRGB(255, 200, 100))
                    end
                })
                ...
                Callback = function()
                    ...
                    if not json or json == "" then
                        netx("External config JSON kosong", 3, Color3.fromRGB(255, 100, 100))
                        return
                    end
                    ...
                    if ok then
                        netx("External config loaded successfully", 3, Color3.fromRGB(100, 255, 100))
                        StatusParagraph:SetContent("Current: External JSON | Autoload: " .. (GetAutoload() or "None"))
                    else
                        netx("Failed to load external JSON", 3, Color3.fromRGB(255, 100, 100))
                    end
                end
            })
